...
#' @param output Character string specifying the ouput directory path. Default
#'   is a tmp folder.
...
.maaslin2_quietly <- function(se,
  ...
  f_quietly <- function(se = se,
    ...
    data <- SummarizedExperiment::assays(se)$humann %>% tibble::rownames_to_column(var = "function_id") %>% tibble::as_tibble()
